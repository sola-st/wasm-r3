...
testNames = ["call-imported", "call-imported-different-modules", "call-imported-params"]
...
async function run() {
    ...
    for (let name of testNames) {
        ...
        const wat2wasm = `wat2wasm ${watPath} -o ${wasmPath}`
        cp.execSync(wat2wasm)
        cp.execSync(`wasabi ${wasmPath} --node --hooks=begin,store,load,call -o ${testPath}`)
        fs.writeFileSync(wasabiRuntimePath, removeLinesWithConsole(fs.readFileSync(wasabiRuntimePath, 'utf-8')))
        ...
        //     continue
        // }
        // let replayTrace = stringify(trace)
        // fs.writeFileSync(replayTracePath, replayTrace)
        ...
        // // console.log('4. Cleanup wasabi runtime')
        // fs.rmSync(wasabiRuntimePath)
        // fs.rmSync(path.join(testPath, 'long.js'))
    }
