...
type TraceStream = Pin<Box<dyn Stream<Item = WasmEvent>>>;
...
impl Trace {
    fn new(trace: TraceStream, module: Module) -> Self {
        Trace { trace, module, shadow_mem: ShadowMemory::new(&module) }
    }

    pub fn from_text(file: File, module: Module) -> Trace {
        let reader = BufReader::new(file);
        Trace::new(
            Box::pin(stream::unfold(reader, |mut reader| async {
    ...
    pub fn from_binary(file: File, module: Module) -> Trace {
        ...
        Trace::new(
            Box::pin(stream::unfold(reader, move |mut reader| async move {
