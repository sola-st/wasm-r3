...
function tracer(runtimePath) {
    ...
    function init() {
        ...
        Wasabi.module.memories.forEach((mem, i) => {
            ...
            if (isImported) {
                ...
                shadowMemories.push(lodash_1.default.cloneDeep(mem.buffer));
            }
        });
        ...
        // Init Globals
        shadowGlobals = lodash_1.default.cloneDeep(Wasabi.module.globals.map(g => g.value));
    ...
    function set_shadow_memory(memIdx, addr, data) {
        for (let i = 0; i < data.length; i++) {
            let shadowArray = new Uint8Array(shadowMemories[memIdx]);
            shadowArray[addr + i] = data[i];
        }
    }
    function mem_content_equals(memIdx, addr, numBytes) {
    ...
    function get_actual_mem(memIdx, addr, numBytes) {
        let data = [];
        for (let i = 0; i < numBytes; i++) {
    ...
    function tableGetEvent(tableidx, idx) {
        ...
        if (!lodash_1.default.isEqual(shadowTable.get(idx), table.get(idx))) {
    ...
    function growShadowMem(memIdx, byPages) {
    ...
    function growShadowTable(tableIdx, amount) {
...
//# sourceMappingURL=tracer-node.cjs.map