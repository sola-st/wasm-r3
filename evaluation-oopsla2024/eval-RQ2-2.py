...
def run_icount(testname, engine, opt):
    ...
    cmd = f'DATA_FILE={data_path} {r3_path}/tests/scripts/run-icount.bash {replay_path} wizeng.x86-64-linux'
    try:        
...
def run_fprofile(testname, engine, opt):
    ...
    replay_path = get_replay_wasm(testname, opt)
    cmd = f'DATA_FILE={data_path} {r3_path}/tests/scripts/run-fprofile.bash {replay_path} wizeng.x86-64-linux'
    try:
        result = subprocess.run(cmd, shell=True, stdout=subprocess.PIPE, text=True)
        with open(data_path, 'r') as f: 
            output = csv.DictReader(f)
...
def run_summarize(testname, engine, opt):
    ...
    replay_path = get_replay_wasm(testname, opt)
    cmd = f'ICOUNT_FILE={icount_path} TICKS_FILE={ticks_path} {r3_path}/tests/scripts/summarize.bash {replay_path}'
    try:
        result = subprocess.run(cmd, shell=True, stdout=subprocess.PIPE, text=True)
