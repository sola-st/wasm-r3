...
export default class Analyser {
  ...
  async start(url: string, { headless } = { headless: false }) {
    if (this.isRunning === true) {
      throw new Error(
        "Analyser is already running. Stop the Analyser before starting again"
      );
    }
    const p_measureStart = createMeasure("start", {
      phase: "record",
      description: `The time it takes start the chromium browser and open the webpage until the 'load' event is fired.`,
    });
    this.isRunning = true;
    let browser = this.options.firefox
      ? firefox
      : this.options.webkit
      ? webkit
      : chromium;
    this.browser = await browser.launch({
      // chromium version: 119.0.6045.9 (Developer Build) (x86_64); V8 version: V8 11.9.169.3; currently in node I run version 11.8.172.13-node.12
      headless,
      args: [
    ...
    p_measureStart();
    this.p_measureUserInteraction = createMeasure("user interaction", {
      phase: "record",
      description: `The time the user interacts with the webpage during recording, from the time the 'loal' event got fired in the browser until the user stopps the recording.`,
    });
    return this.page;
  }
  ...
  async stop(): Promise<AnalysisResult> {
    if (this.isRunning === false) {
      throw new Error(
        "Analyser is not running. Start the Analyser before stopping"
      );
    }
    this.p_measureUserInteraction();
    const p_measureStop = createMeasure("stop", {
      phase: "record",
      description: `The time it takes to stop the recording, from when the user stopped the recording until all data is downloaded from the browser and the browser is closed.`,
    });
    this.contexts = this.contexts.concat(this.page.frames());
    const p_measureDataDownload = createMeasure("data download", {
      phase: "record",
      description: `The time it takes to download all data from the browser.`,
    });
    const p_measureTraceDownload = createMeasure("trace download", {
      phase: "record",
      description: `The time it takes to download all traces from the browser.`,
    });
    const traces = (await this.getResults()).map((t) =>
      trimFromLastOccurance(t, "ER")
    );
    p_measureTraceDownload();
    const p_measureBufferDownload = createMeasure("buffer download", {
      phase: "record",
      description: `The time it takes to download all wasm binaries from the browser.`,
    });
    ...
    p_measureStop();
    return traces.map((result, i) => ({
      result: result,
      wasm: originalWasmBuffer[i],
      statistic: stats[i],
    }));
  }
  ...
  private async getResults() {
    const results = await Promise.all(
      this.contexts.map(async (c) => {
        const p_measureTraceDownload = createMeasure(
          `trace download from context: ${c.url()}`,
          {
            phase: "record",
            description: `The time it takes to download the trace from the browser context: ${c.url}.`,
          }
        );
  ...
  private async getBuffers() {
    const originalWasmBuffer = await Promise.all(
      this.contexts.map(async (c) => {
        const p_measureBufferDownload = createMeasure(
          `buffer download from context: ${c.url()}`,
          {
            phase: "record",
            description: `The time it takes to download the wasm binary from the browser context: ${c.url}.`,
          }
        );
  ...
  private async constructInitScript() {
    ...
    analysisScript = analysisScript
      ...
      .join("");
    analysisScript = analysisScript
      .split("exports.default = setupAnalysis;")
      .join("");
    analysisScript = analysisScript
      .split("exports.stringifyTrace = void 0;")
      .join("");
    analysisScript = analysisScript
      .split("exports.stringifyTrace = stringifyTrace;")
      .join("");
    ...
    analysisScript = analysisScript
      .split("exports.default = Analysis;")
      .join("");
