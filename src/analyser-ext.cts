...
export default class Analyser {
    ...
    private outputStream: fss.WriteStream
    ...
    async start(url: string, { headless } = { headless: false }) {
        ...
        // this.startTraceDownload()
        this.performanceTraceLocal.push(p_start.stop())
        this.performanceTraceLocal.push(new PerformanceEntry('user interaction'))
        return this.page
    }

    private startTraceDownload() {
        this.outputStream = fss.createWriteStream('trace.r3')
        this.downloadInterval = setInterval(async () => {
            const analysisResults: string[] = (await this.page.evaluate(() => {
                //@ts-ignore
                return analysis.map((r) => { 
                    const result = r.getResult().toString()
                    r.clear()
                    return result
                })
            }))
            if (analysisResults.length !== 0) {
                if (analysisResults[0] !== undefined) {
                    this.outputStream.write(analysisResults[0])
                }
            }
        }, 100)
    }
    ...
    async stop(): Promise<AnalysisResult> {
        ...
        await fs.writeFile('trace-main.r3', analysisResults[0])
        ...
        await fs.writeFile('trace.r3', workerResults[0])
