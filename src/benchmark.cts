...
export default class Benchmark {
  ...
  async save(
    ...
    options = { trace: false, rustBackend: false }
  ) {
    const p_measureSave = createMeasure("save", {
      phase: "replay-generation",
      description:
        "The time it takes to save the benchmark to the disk. This means generating the intermediate representation code from the trace and streaming it to the file, as well as saving the wasm binaries.",
    });
    ...
    await Promise.all(
      this.record.map(async ({ binary, trace }, i) => {
        ...
        if (options.rustBackend === true) {
          const p_measureCodeGen = createMeasure("rust-backend", {
            phase: "replay-generation",
            description: `The time it takes for rust backend to generate javascript`,
          });
          execSync(
            `./crates/target/release/replay_gen ${diskSave} ${path.join(
              binPath,
              "replay.js"
            )}`
          );
          ...
          const code = await new Generator().generateReplayFromStream(
            fss.createReadStream(diskSave)
          );
          p_measureCodeGen();
          const p_measureJSWrite = createMeasure("string-gen", {
            phase: "replay-generation",
            description: `The time it takes to stream the replay code to the file for subbenchmark ${i}`,
          });
          await generateJavascript(
            fss.createWriteStream(path.join(binPath, "replay.js")),
            code
          );
          p_measureJSWrite();
        }
