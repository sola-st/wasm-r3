...
timeout_set = ['wasmedge#3018', 'wasmedge#3019', 'wasmedge#3057', 'wasmedge#3076', 'wamr#2789']
...
for testname in testset:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            ...
            for fidx in fidx_range:
                if not metrics[testname]["rq1"].get(str(fidx)) or not metrics[testname]["rq1"].get(str(fidx)).get('sliced_file_size') or metrics[testname]["rq1"].get(str(fidx))['sliced_file_size'] == "fail":
